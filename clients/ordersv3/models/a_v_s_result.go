// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// AVSResult AVS Results
//
// Address Verification Service (AVS) result from the payment gateway.
//
// swagger:model AVSResult
type AVSResult struct {

	// AVS code.
	Code string `json:"code,omitempty"`

	// AVS message.
	Message string `json:"message,omitempty"`

	// AVS Code for postal matching result.
	PostalMatch string `json:"postal_match,omitempty"`

	// AVS Code for street matching result.
	StreetMatch string `json:"street_match,omitempty"`
}

// Validate validates this a v s result
func (m *AVSResult) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *AVSResult) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AVSResult) UnmarshalBinary(b []byte) error {
	var res AVSResult
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
